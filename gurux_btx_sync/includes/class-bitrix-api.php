...
class YeisonBTX_Bitrix_API {
    ...
    /**
     * Limpiar todos los tokens (útil al cambiar de cuenta)
     */
    ...
    /**
     * Detectar si el token es inválido por cambio de cuenta
     */
    public function is_token_invalid_for_domain() {
        if (empty($this->config['access_token'])) {
            return false; // No hay token
        }
        
        // Hacer test simple de conectividad
        $response = $this->api_call('app.info');
        
        if (isset($response['error'])) {
            $error_codes = array('invalid_token', 'expired_token', 'invalid_grant');
            
            if (in_array($response['error'], $error_codes)) {
                return true;
            }
        }
    ...
    /**
     * Auto-limpiar tokens si son inválidos
     */
    public function auto_clean_invalid_tokens() {
        if ($this->is_token_invalid_for_domain()) {
            yeison_btx_log('🔄 Auto-limpiando tokens inválidos', 'info');
            $this->clear_tokens();
            return true;
        }
        return false;
    ...
    public function test_connection() {
        ...
        if ($response && isset($response['result'])) {
            ...
            switch ($response['error']) {
                case 'invalid_token':
                case 'expired_token':
                    $result['message'] = 'Token inválido o expirado. Probablemente cambiaste de cuenta de Bitrix24.';
                    $result['needs_reauth'] = true;
                    
                    // Auto-limpiar tokens inválidos
                    yeison_btx_log('🧹 Auto-limpiando tokens por cambio de cuenta', 'warning');
                    $this->clear_tokens();
                    break;
                    
                case 'invalid_grant':
                    $result['message'] = 'Credenciales inválidas. Revisa Client ID y Client Secret.';
                    $result['needs_reauth'] = true;
                    break;
                    
                default:
                    $result['message'] = 'Error en la conexión: ' . ($response['error_description'] ?? $response['error']);
            }
            
            yeison_btx_log('❌ Test de conexión fallido', 'error', array(
                'error' => $response['error'],
                'description' => $response['error_description'] ?? 'No description'
            ));
            
        } else {
            $result['message'] = 'Respuesta inválida del servidor';
            yeison_btx_log('❌ Respuesta inválida en test', 'error', array('response' => $response));
        }
    ...
    /**
     * Verificar si está autorizado (mejorado)
     */
    public function is_authorized() {
        ...
        // Verificar si el token es válido para el dominio actual
        if ($this->is_token_invalid_for_domain()) {
            return false;
        }
