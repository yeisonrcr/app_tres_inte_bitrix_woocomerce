...
class YeisonBTX_Data_Mapping {
    ...
    private function get_default_status_mapping() {
        ...
        $current_pipeline = yeison_btx_get_selected_pipeline();
        
        // Intentar obtener mapeo específico del pipeline
        $pipeline_mapping = yeison_btx_get_pipeline_status_mapping($current_pipeline);
        
        if (!empty($pipeline_mapping)) {
            return $pipeline_mapping;
        }
        
        // Si no hay mapeo para el pipeline, usar el general
